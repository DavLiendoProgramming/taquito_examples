...
(* declaring the type of SC storage. It keeps the overall quantity of tokens and big_map data structure
 that ensures the connection between user balances and public addresses *)
type storage is
  record [
    totalSupply     : amt;
...
(* declaring the alias for the return method that will return operations. In short contracts one can do 
without it, yet it is easier to describe the return type just once in contracts with several pseudo-entry 
points and then use it in every function *)
...
// totalSupply doesn’t use michelson_pair as the first input param is the empty value of unit 
// will be the first anyway after being sorted by Michelson compiler
...
///gets the input parameter of address type and the value of storage from the smart contract
  ...
  block {
    ...
    (* checking if the storage has the user account: if no, leave acct empty 
    with the previous block’s value; if yes, assign the value from the storage 
    to acct. The function returns the acct value *)
  ...
  (*asks the user how many tokens they allow it to transfer to another address. It gets the user’s address, 
  the contract address (spender), and the storage state, and returns the argument amt, which is the number of 
  tokens subject to spending.*)

  function getAllowance (const ownerAccount : account; const spender : address; const s : storage) : amt is
  (* if the user has allowed to send a certain amount of tokens, the function assigns the amount to amt. If not,
   the number of tokens equals zero *)
  case ownerAccount.allowances[spender] of
    Some (amt) -> amt
  | None -> 0n
  end;

  (* gets sender and recipient addresses from the user along with the number of tokens subject to sending and the storage state*)
...
(*requests the confirmation of the number of tokens the spender address can send from the sender
address. For example, a blockchain app (spender) requests the user (sender) for permission to send tokens.*)
...
function approve (const spender : address; const value : amt; var s : storage) : return is
  ...
  (*getBalance, getAllowance, and getTotalSupply are so-called view functions.
   They return the requested number not to the sender but to a proxy contract. 
   The latter allows apps to get data from user contracts and show them in the interface.*)
